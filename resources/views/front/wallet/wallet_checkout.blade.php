...
<section id="wishlist-home" class="wishlist-home-main-block">
    <div class="container">
        <h1 class="wishlist-home-heading text-white">{{ __('frontstaticword.Wallet') }} {{ __('frontstaticword.Checkout') }}</h1>
    </div>
</section>
	            	...
	            	<div class="checkout-pricelist">
		            	<ul>
		            		@if($gsetting['currency_swipe'] == 1)
		            			<li><h1 class="checkout-total">{{ __('frontstaticword.Total') }}: <i class="{{ $currency->icon }}"></i>{{ strip_tags($amount) }}</h1></li>
		            			
		            		@else
		            			<li><h1 class="checkout-total">{{ __('frontstaticword.Total') }}: {{ strip_tags($amount) }}<i class="{{ $currency->icon }}"></i></h1></li>
							        ...
							        <div class="card-body">
		                            	...
		                            	<div class="payment-proceed-btn">
		                            		<form action="{{ route('wallet.paypal') }}" method="POST" autocomplete="off">
		                            			...
		                            			<button class="btn btn-primary" title="{{ __('checkout') }}" type="submit">{{ __('frontstaticword.Proceed') }}</button>
		                            		</form>
