...
class MainController extends Controller
{
    ...
    public function home(Request $request){
        ...
        if(isset($category_slider))
        {
            foreach($category_slider->category_id as $cats)
            {
                ...
                foreach($catee->courses as $course)
                {
    ...
    public function mycourses(Request $request)
    {
        ...
        if(isset($enroll)){
            foreach ($enroll as $enrol) {
                $course = Course::where('status', 1)->where('id', $enrol->course_id)
                ...
                $enroll_details[] = array(
                    'title' => isset($course->title) ? $course->title : $bundle->title,
                    'enroll' => $enrol,
                    'course' => $course,
                    'bundle' => $bundle,
    ...
    public function categoryPage(Request $request, $id, $name)
    {
        ...
        $subcategory = SubCategory::where('status', 1)->where('category_id', $category->id)->get();
    ...
    public function subcategoryPage(Request $request, $id, $name)
    {
        ...
        $course = Course::where('status', 1)->where('subcategory_id', $category->id)->paginate(10);
        $result = array(
            'id' => $category->id,
            'title' => array_map(function ($lang) {
                            return trim(preg_replace("/\r\n|\r|\n/",'',strip_tags(html_entity_decode($lang))));
    ...
    public function childcategoryPage(Request $request, $id, $name)
    {
        ...
        $course = Course::where('status', 1)->where('childcategory_id', $category->id)->paginate(2);
        $result = array(
            'id' => $category->id,
            'title' => array_map(function ($lang) {
                            return trim(preg_replace("/\r\n|\r|\n/",'',strip_tags(html_entity_decode($lang))));
